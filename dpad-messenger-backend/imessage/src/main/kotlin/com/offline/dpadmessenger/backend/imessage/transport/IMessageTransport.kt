...
/**
 * The protocol boundary for iMessage. Everything above this line (session,
 * repository, UI) is transport-agnostic; everything below is "how do we
 * actually talk to Apple."
 *
 * Three implementations share this contract (IMESSAGE_NATIVE_BACKEND_PLAN.md
 * §2.6 / §3):
...
interface IMessageTransport {
    ...
    /** iMessage tapback (reaction). [emoji] is the rendered tapback; classic
    ...
    /** iMessage edit (15-min window). Returns false if rejected/expired. */
    suspend fun editMessage(chatGuid: String, targetGuid: String, newText: String): Boolean

    /** iMessage unsend (2-min window). Returns false if rejected/expired. */

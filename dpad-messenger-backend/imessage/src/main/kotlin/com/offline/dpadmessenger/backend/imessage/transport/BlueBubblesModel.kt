...
/**
 * Authoritative iMessage protocol constants, modeled on the actual
...
/**
 * iMessage **tapbacks**, with BlueBubbles' `associatedMessageType` integer
 ...
 *
 * The six classic tapbacks map to fixed emoji for the UI; modern iMessage also
 * allows arbitrary-emoji tapbacks (sticker reactions), which carry their own
 * type and pass through as the raw emoji (IMESSAGE_NATIVE_BACKEND_PLAN.md §5).
 */
...
/**
 * Apple Cocoa epoch helpers. iMessage timestamps in the raw protocol (and in
 ...
 *  - group: `iMessage;+;chat4827...` (an opaque group id)
...
/** iMessage services: blue (iMessage) vs green (SMS/RCS relayed). */

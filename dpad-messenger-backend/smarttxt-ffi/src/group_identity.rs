...
//
// rustpush's `normalize_sms_handle` turns a bare all-digit SMS handle into E.164 by
// prefixing '+' and stopping there. When the carrier delivers a national-format number
// — which is what a NANP carrier does for a domestic text — "4097821402" becomes the
// bogus "+4097821402": country code 40, Romania. `canon` cannot undo it, because its
// "10 bare digits get a +1" rule only fires on a handle that has NO '+' yet.
//
// The damage splits every green thread in two: the user's outbound echoes arrive from
// the paired iPhone already in E.164 and key to "+1…", while the other party's replies
// key to "+…", so a reply typed into the wrong one is addressed to a number that does
// not exist. (2026-08-30 bundle: 4 phantom rooms, 18 stranded messages and a photo.)
//
// Repair is only possible with context — WHICH country the number was national to — so
// none of it belongs in `canon`, which is deliberately context-free. These helpers take
// the account's own number and are used by `lib.rs::repair_relayed_sms`, which applies
// them only to a message it has PROVEN was damaged (see there).
...
/// The bogus form `normalize_sms_handle` would produce for a known-good NANP number:
/// "+18048334449" → "+8048334449". Computing the damage FORWARD from a number we
/// already trust lets us match a mangled participant by equality instead of guessing
/// backward at what its country code used to be — which is how the user's own handle
/// and the message sender are repaired exactly, with no heuristic at all.
///
/// `None` for anything that is not NANP: we only know how many digits to drop there.
pub fn bare_plus_form(canon_e164: &str) -> Option<String> {
    is_nanp_e164(canon_e164).then(|| format!("+{}", &canon_e164[2..]))
}

/// Re-qualify a handle that arrived as '+' plus a bare national number.
///
/// Deliberately narrow. It fires ONLY when the account itself is NANP and the handle is
/// '+' followed by exactly ten digits in NANP shape (area and exchange codes both start
/// 2-9) — a combination that is never a valid NANP number, since those are '+1' plus
/// ten. Everything else is returned untouched.
///
/// It has to be this narrow: a legitimate ten-digit E.164 exists elsewhere (Norway
/// "+4712345678", Denmark "+4512345678"), and without a full libphonenumber we cannot
/// tell one of those from a NANP number whose country code was dropped. The caller adds
/// the second guard — it only asks for this on a message whose sender is already known,
/// from the relay's own fully-qualified copy, to have arrived without a country code.
pub fn requalify_national(canon_handle: &str, my_canon: &str) -> String {
    if !is_nanp_e164(my_canon) {
        return canon_handle.to_string(); // not a NANP account: we have no country to add
    }
...
mod tests {
    ...
    // ---- relayed-SMS handle repair (Hannah Benedict, 2026-08-30 bundle) ----------
    //
    // Real values from the log: her account is +14098932801, the other party is
    // +14097821402, and every inbound SMS arrived with `h` = "4097821402", which
    // rustpush turned into "+4097821402".

    #[test]
    fn canon_cannot_undo_a_manufactured_plus() {
        // The regression itself: once rustpush has glued the '+' on, canon's
        // "10 bare digits get a +1" rule no longer fires. This is why the repair
        // cannot live in canon.
        assert_eq!(canon("4097821402"), "+14097821402"); // bare: correct
        assert_eq!(canon("tel:+4097821402"), "+4097821402"); // damaged: stays damaged
    }

    #[test]
    fn nanp_shape_is_recognised() {
        ...
        assert!(!is_nanp_e164("+1809483344490")); // too long
    }

    #[test]
    fn damage_computed_forward_matches_the_wire() {
        // The exact strings from the log, in both directions.
        assert_eq!(bare_plus_form("+14097821402").as_deref(), Some("+4097821402"));
        assert_eq!(bare_plus_form("+14098932801").as_deref(), Some("+4098932801"));
        assert_eq!(bare_plus_form("+18324058464").as_deref(), Some("+8324058464"));
        assert_eq!(bare_plus_form("+4712345678"), None); // not NANP: don't guess
        assert_eq!(bare_plus_form("sav@icloud.com"), None);
    }
    ...
    fn requalify_leaves_everything_else_alone() {
        let me = "+14098932801";
        // Already correct.
        assert_eq!(requalify_national("+14097821402", me), "+14097821402");
        // Emails and short codes.
        assert_eq!(requalify_national("sav@icloud.com", me), "sav@icloud.com");
        assert_eq!(requalify_national("+30295", me), "+30295");
        assert_eq!(requalify_national("+61607", me), "+61607");
        // Area/exchange codes that cannot be NANP.
        assert_eq!(requalify_national("+0123456789", me), "+0123456789");
        assert_eq!(requalify_national("+4091234567", me), "+4091234567"); // exchange starts 1
    }

    #[test]
    fn requalify_never_fires_for_a_non_nanp_account() {
        // A Norwegian account must never have its own countrymen rewritten to +1.
        // This is the guard that keeps the narrow rule safe outside the NANP.
        let me = "+4712345678";
    ...
    #[test]
    fn repaired_handles_key_to_the_same_room_as_outbound() {
        // The whole point: after repair, the inbound room key and the key her own
        // outbound echo produces are the same string, so the thread stops splitting.
        let me = "+14098932801";
        let inbound = requalify_national(&canon("tel:+4097821402"), me);
        let outbound_echo = canon("tel:+14097821402"); // `h` on her own relayed sends
        assert_eq!(inbound, outbound_echo);

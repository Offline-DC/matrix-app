 ...
 * refresh, but for IDS (SMARTTXT_NATIVE_BACKEND_PLAN.md §2.2/§4/§8).
 *
 ...
 * the launcher's other chains use, and [SmartTxtRepository.status] gives the
 * field a visible "stuck renewal" surface.
 */
class SmartTxtRenewalWorker(
    appContext: Context,
    params: WorkerParameters,
) : CoroutineWorker(appContext, params) {

    override suspend fun doWork(): Result {
        Log.i(TAG, "running SmartTxt renewal")
        return when (val r = SmartTxtRepository.renew(applicationContext)) {
    ...
    companion object {
        ...
        private const val WORK_NAME = "smarttxt_renewal"
        ...
        fun schedule(context: Context) {
            val constraints = Constraints.Builder()
                ...
                .build()
            val request = PeriodicWorkRequestBuilder<SmartTxtRenewalWorker>(
                12, TimeUnit.HOURS,

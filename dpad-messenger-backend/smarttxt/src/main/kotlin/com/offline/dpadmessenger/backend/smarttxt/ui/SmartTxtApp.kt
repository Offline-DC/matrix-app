...
import com.offline.dpadmessenger.backend.smarttxt.SmartTxtMessageRepository
...
fun SmartTxtApp(
    ...
    when (status) {
        SmartTxtStatus.REGISTERED -> {
            val repository = remember { SmartTxtRepository.create(context) }
            ...
            DpadMessengerApp(
                ...
                // "Re-link phone" and "Days since last link" are intentionally
                // left off for the demo: omitting onRelink/onFreshRelink/
                // linkAgeDays (all default to null) hides both Settings rows.
                // There's no real ~2-week IDS session to renew here.

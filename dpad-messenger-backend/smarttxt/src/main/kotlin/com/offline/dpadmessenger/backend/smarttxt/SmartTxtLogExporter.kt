...
internal object SmartTxtLogExporter {
    ...
    private fun StringBuilder.appendReadingGuide() {
        ...
        append("#   'CATCHUP repo: save'  — each whole-store write: count / rooms / durMs\n")
        append("#   'CATCHUP ingest:'     — decrypted vs admitted vs outsideWindow. A high\n")
        append("#     discardedPct means most of the elapsed sync was spent decrypting\n")
        append("#     messages older than 3 days, which the user never sees. That is the\n")
        append("#     usual reason a sync takes minutes with little to show for it.\n")
        append("#   'CATCHUP pacer:'      — drainPressure, i.e. did the QUEUE back up. This\n")
        append("#     is NOT the same as being busy: a discard-heavy sync keeps the queue\n")
        append("#     empty throughout. Spinner + save coalescing follow ingest, not this.\n")

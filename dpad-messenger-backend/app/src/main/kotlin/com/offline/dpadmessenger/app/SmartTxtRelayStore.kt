...
/**
 * Persists the SmartTxt relay connection (the daemon's URL + optional bearer
 * token) so the app reconnects on boot — the analogue of [SignalAccountStore].
 *
 * The daemon (`smarttxt-relay`) performs the actual Apple registration: it reads
 ...
 * endpoint.
 */
class SmartTxtRelayStore(context: Context) {

    private val prefs =
        context.getSharedPreferences("smarttxt_relay", Context.MODE_PRIVATE)

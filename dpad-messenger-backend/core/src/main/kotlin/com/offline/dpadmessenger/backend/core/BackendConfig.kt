 ...
 * variant, or the native SmartTxt backend.
 */
sealed class BackendConfig {
    ...
    /**
     * **Direct** native SmartTxt mode (mirrors [SignalDirect]). The repository
     * talks to rustpush in-process via `RustPushBridge`; there is no Matrix or
     * Conduit between the UI and Apple. Fulfilled by the `:smarttxt` module's
     * `SmartTxtBackendFactory`.
     *
     ...
     * `ValidationDataRelay`, and the chat UI is fed SmartTxt-flavoured demo
     * data. The seam is shaped so the real native build drops in without
     * touching the UI. See ABSINTHE_REVERSE_ENGINEERING.md and
     * SMARTTXT_NATIVE_BACKEND_PLAN.md §2.5/§2.6.
     */
    data object SmartTxtNative : BackendConfig()
}

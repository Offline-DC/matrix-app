# Native SmartTxt backend plan (`:smarttxt` in dpad-messenger-backend)
...
is about **replacing** that black box: bringing SmartTxt in as a first-class
...
> rustpush ourselves leaves SmartTxt registration non-functional (panics at
...
- **Full native.** Copy OpenBubbles' rustpush + dumb-file/hardware-info
  handling into our own `:smarttxt` module. Goal end-state: the TCL Flips no
...
- We already own the chat UI. Hosting SmartTxt in `dpad-messenger` means one
  consistent messenger experience across Signal / Google Messages / SmartTxt
...
└─────────────────┘                                       └──────────────────┘
          ...
          │ MessageRepository
┌─────────┴───────────────────┐
│ SmartTxtMessageRepository    │  implements the UI contract
│ RustPushBridge (JNI/UniFFI)  │  thin Kotlin ↔ Rust surface
│ APNsForegroundService        │  keeps the push connection alive
│ SmartTxtAccountStore         │  encrypted creds + hardware info ("dumb file")
│ SmartTxtRenewalWorker        │  periodic offline re-registration (Mac mode)
└──────────────────────────────┘
```
...
New Gradle module: `dpad-messenger-backend/smarttxt`, namespace
`com.offline.dpadmessenger.backend.smarttxt`, consumed by `dumb-down-launcher`
...
This is the part that differentiates SmartTxt from Signal/gmessages. The good
...
we invent. Our `SmartTxtAccountStore` just holds this blob (encrypted) and
...
4. **IDS registration** — `register(...)` (`src/ids/user.rs`). Combines
   ...
   that actually authorizes SmartTxt. **Renewal = re-run steps 3–4 on a
   schedule** (`SmartTxtRenewalWorker`).
...
- **Apple ID credentials + 2FA**, once, for step 2. Whether that happens in-app
  ...
  must land in `SmartTxtAccountStore` alongside the dumb file. Registration is
...
(gmessages). Ongoing obligations: the renewal schedule (`SmartTxtRenewalWorker`,
...
never completes; no SmartTxt. This is OpenBubbles' withheld secret sauce (the
...
# write smarttxt-ffi/ wrapper crate depending on rustpush
...
```
dpad-messenger-backend/smarttxt/src/main/jniLibs/
├── arm64-v8a/      libsmarttxt_ffi.so
└── armeabi-v7a/    libsmarttxt_ffi.so
```

`RustPushBridge` calls `System.loadLibrary("smarttxt_ffi")` and owns a tokio
...
| File | Role | Analogue |
|---|---|---|
| `SmartTxtBackendFactory.kt` | `create(ctx, BackendConfig.SmartTxtNative) → MessageRepository?`; null until seeded | `SignalBackendFactory` |
| `SmartTxtConfig.kt` | host-set `messengerActivityClassName` for notif tap target | `GoogleMessagesConfig` / `SignalConfig` |
| `SmartTxtAccountStore.kt` | EncryptedSharedPreferences: hardware info + IDS keys + auth token + renewal timestamp | `SignalAccountStore` |
| `RustPushBridge.kt` | `external fun` JNI surface (or UniFFI-generated) over rustpush | `ConduitBinaryLoader` |
| `SmartTxtMessageRepository.kt` | implements `MessageRepository`; maps rustpush events ↔ UI model | `SignalMessageRepository` |
...
| `SmartTxtRenewalWorker.kt` | periodic re-registration from hardware info (re-runs `generate_validation_data` + `register`; needs Apple connectivity, not the Mac) | (new — no analogue) |
| `SmartTxtNotifier.kt` | posts incoming-message notifications, opens host Activity | `GoogleMessagesNotifier` |
| `ui/SmartTxtApp.kt` | `SmartTxtApp()` composable: status/setup gate → `DpadMessengerApp` | `GoogleMessagesApp` |
| `ui/SmartTxtSetupScreen.kt` | "import dumb file + account state" / SmartTxt status screen | `SignalLinkScreen` |

Add `BackendConfig.SmartTxtNative` to `:core`'s sealed `BackendConfig`, and an
`OpenBubbles/SmartTxt` `Target` enum value if we reuse `StorageCleanupOps`.

## 5. Mapping rustpush ↔ `MessageRepository`

The UI contract (`MessageRepository`) is already SmartTxt-shaped — it has
...
- **`observeRoomSummaries` / `observeMessages`** ← rustpush chat + message
  streams. SmartTxt "chats" (1:1 + group) → `Room`/`RoomSummary`.
- **`sendMessage(replyToId)`** → rustpush send; SmartTxt inline replies map to
  `replyToId`.
- **`toggleReaction(emoji)`** ↔ SmartTxt **tapbacks**. Note: tapbacks are a
  fixed set (love/like/dislike/laugh/emphasize/question) plus, on modern
  SmartTxt, arbitrary emoji. Map the six classic tapbacks to fixed emoji and
  pass arbitrary ones through.
- **`editMessage` / `deleteMessage`** ↔ SmartTxt **edit** / **unsend**
  (15-min / 2-min windows; surface failures as no-ops per the interface
  contract).
- **`markRoomRead`** → SmartTxt read receipts (respect the per-chat
...
SmartTxt-specific things with no UI slot yet (defer / extend the model later):
...
- **Phase A — plumbing.** Create `:smarttxt`, wire into launcher composite
  build, `MessengerActivity` hosts `SmartTxtApp()`, mock `MessageRepository` so
  the chat screens render with demo data. No native code yet. (Same as
  gmessages Phase A.)
- **Phase B — rustpush `.so` + APNs connect.** Write the `smarttxt-ffi`
...
- **Phase C — seed + register.** Import the dumb file + account state into
  `SmartTxtAccountStore`; drive rustpush registration (`activate` →
  `authenticate_apple` → `generate_validation_data` → `register`);
  `SmartTxtRenewalWorker` schedules periodic re-registration (Mac mode; needs
  Apple connectivity, not the Mac). Success = "registered with SmartTxt"
  status, device shows on the Apple ID's device list.
- **Phase D — receive + send.** Wire rustpush inbound events → repo streams →
  UI; `sendMessage` outbound; `SmartTxtNotifier` for incoming. Success =
  two-way text with a real SmartTxt contact in our UI.
...
- `MessengerActivity` (or a sibling `SmartTxtMessengerActivity`, like
  `SignalMessengerActivity`) is a thin shell hosting `SmartTxtApp()`.
...
- **rustpush API churn.** Same warning as `matrix-rust-sdk` in the backend
  ...
  Pin a commit, isolate all churn in the `smarttxt-ffi` wrapper crate, budget
...
- **Validation renewal correctness.** Mac-mode renewal (re-running
  ...
  the linchpin.
  If `SmartTxtRenewalWorker` misses its window (Doze, killed worker), the device
  silently de-registers. Needs the same robustness as the existing WorkManager
  chains, plus a visible "SmartTxt status" surface so a stuck renewal is
